...
t_turn = 1;             % s when to start turning
t_brake = 4;            % s when to start braking
...
%Given variables    
ui = 26.8224;           % 60 mph in m/s
mu = 0.56;               % Given friction coefficient

%Chosen step steer
delta_f = 0*pi/180;     % rad CHOOSE A STEP STEER THAT RESULTS IN 0.3g TURN
...
car = struct('tspan',tspan, 'm', m, 'a', a, 'L', L, 'b', b, 'Izz', Izz, 'C_f', C_f, 'C_r', C_r, 'g', g, 'mu', mu, ...
    ...
    't_turn',t_turn,'t_brake',t_brake);

% Initial conditions
r0 = 0; v0 = 0; glob_X0 = 0; glob_Y0 = 0; psi0 = 0; phi0 = 0; phi_dot0 = 0; u0 = ui; theta0 = 0; theta_dot0 = 0;  %Velocity at beginning is given
...
[f1, f2, f3, f4, f5, f6, f7, f8, f9, f10, f11, f12, f13, f14] = plot_all(tspan, X, a_y, a_x, N_all, Fx_all, BF, delta_f, LU, Fy_all);
...
%Patch plot:
i_turn = find(tspan == t_turn);
i_brake = find(tspan == t_brake);
...
figure(99)
    ...
    %xline(X(i_turn,3),'b--')
    xline(X(i_brake,3),'r--')
    legend('Patched Trajectory','Braking starts')
    box on
...
%% Computing functions
function [Xdot, a_y, a_x, N_all, Fx_all, BF, delta_f, LU, mus, Fy_all] = car_modelNL(X, t, car) %n is the iteration number
    tspan = car.tspan; m = car.m; a = car.a; L = car.L; b = car.b; Izz = car.Izz; C_f = car.C_f; C_r = car.C_r; g = car.g; mu = car.mu;
    ...
    K_dbar = car.K_dbar; t_turn = car.t_turn; t_brake = car.t_brake;
    ...
    if t < t_turn  %Don't do anything for the first t < t_turn
        delta_f = 0;
        Q = 0;
    else
        delta_f = delta_f;
    ...
    %If we are braking:
    if t > t_brake %Break when ~steady state, at t > t_brake
        decel = 0;  %0.39 is no lockups for task 7.1
    ...
    [mu_fL, mu_fR, mu_rL, mu_rR] = fric_coeff(glob_Y, psi, car, glob_X);
    mus = [mu_fL, mu_fR, mu_rL, mu_rR];
...
F_check = -mu*N*cos(alpha);
    if abs(F_check) < BF
        ...
        Fx = F_check;
        LU = 0.5;
    elseif (mu*N)^2 < (BF^2 + Fy^2) %SKAL DET VÆRE ET NYT IF-STATEMENT?
...
% marching forward through time
    for n = 1:nt-1
        ...
        [~, a_y(n+1,:),a_x(n+1,:), N_all(n+1,:), Fx_all(n+1,:), BF(n+1,:), delta_f(n+1,:), LU(n+1,:), mus(n+1,:), Fy_all(n+1,:)] = dYdt(Yn, t(n), car);  %Extraction lateral accelerations
        k1 = dYdt(Yn,           t(n), car);  
        k2 = dYdt(Yn+(h/2)*k1,  t(n)+(h/2), car);
        k3 = dYdt(Yn+(h/2)*k2,  t(n)+(h/2), car);
        k4 = dYdt(Yn+h*k3,      t(n)+h, car);
        Y(n+1,:) = Yn + h/6*k1 + h/3*(k2+k3) + h/6*k4; 
        if abs(atan2(Y(n,2),Y(n,8)) .* 180/pi) > 90 | all(diff(mus) == 0)
            %If sideslip gets above 90 deg STOP or all wheels are on the
            %same side
            Y(n+2:end,:) = [];
            if abs(atan2(Y(n,2),Y(n,8)) .* 180/pi) > 90
                fprintf("Sideslip angle is above 90 deg \n")
            elseif all(diff(mus) == 0)
                fprintf("All wheels are on the same side")
            end
            return
        end
...
function [mu_fL, mu_fR, mu_rL, mu_rR] = fric_coeff(Y, psi, car, glob_X)
...
FR_wy=(Y+r_a*(cos(v_a)*sin(psi)+sin(v_a)*cos(psi))); %Flipped left and rights to take the flipped y-axis of the patched trajectory into account
...
if  (FL_wy < 0) %If the front left positive then i
    mu_fL = 0.8;  
else 
    mu_fL = 0.1;
    fprintf("Front left reached right side at X=%.5f \n", glob_X)
end

if  (FR_wy < 0)
    mu_fR = 0.8;
    fprintf("Front right reached left side at X=%.5f \n", glob_X)
else 
    mu_fR = 0.1;
end
...
if  (RL_wy < 0)
    ...
    mu_rL = 0.1;
    fprintf("Rear left reached right side at X=%.5f \n", glob_X)
end

if  (RR_wy < 0)
    mu_rR = 0.8;
    fprintf("Rear right reached left side at X=%.5f \n", glob_X)
else 
    mu_rR = 0.1;
end
...
%% Plotting functions
function [f1, f2, f3, f4, f5, f6, f7, f8, f9, f10, f11, f12, f13, f14] = plot_all(tspan, X, a_f, a_x, N_all, Fx_all, BF, delta_f, LU, Fy_all)
g = 9.8;
size = 1.5;
...
f1 = figure(1);
    ...
    hold off
    xlim([0, tspan(end)])
    title('Long. acceleration vs Time');
    xlabel('Time (s)');
...
f4 = figure(4);
    hold on
    plot(tspan, X(:,1).*180/pi,'LineWidth',size);
    %legend(,'Location','southeast')
    hold off
    xlim([0, tspan(end)])
...
f5 = figure(5);
    ...
    hold off
    xlim([0, tspan(end)])
    title('Lat. acceleration vs Time');
    xlabel('Time (s)');
...
f14 = figure(14);
    ...
    legend('Front left','Front right','Rear left','Rear right', 'Location', 'southwest');
    title('Lateral forces on front and back');
    grid on;
    box on;
end
...
for i=1:length(car_r_n)
    j=round(car_r_n(i));
    car_p(i,1)=X(j)+r_a*(cos(v_a)*cos(psi(j))-sin(v_a)*sin(psi(j)));
    car_p(i,2)=Y(j)+r_a*(cos(v_a)*sin(psi(j))+sin(v_a)*cos(psi(j)));
    car_p(i,3)=X(j)+r_b*(-cos(v_b)*cos(psi(j))-sin(v_b)*sin(psi(j)));
    car_p(i,4)=Y(j)+r_b*(-cos(v_b)*sin(psi(j))+sin(v_b)*cos(psi(j)));
    car_p(i,5)=X(j)+r_b*(-cos(v_b)*cos(psi(j))+sin(v_b)*sin(psi(j)));
    car_p(i,6)=Y(j)+r_b*(-cos(v_b)*sin(psi(j))-sin(v_b)*cos(psi(j)));
    car_p(i,7)=X(j)+r_a*(cos(v_a)*cos(psi(j))+sin(v_a)*sin(psi(j)));
    car_p(i,8)=Y(j)+r_a*(cos(v_a)*sin(psi(j))-sin(v_a)*cos(psi(j)));
end
